    ...
    export default function CreateProgramWithSteps({ auth }) {
        ...
        const [initialMaterials] = useState([
            { 
                id: 1, 
                type: 'document', 
                title: '', 
        ...
        const [programData, setProgramData] = useState({
            ...
            category: '',
            difficulty: 'intermediate',
            is_active: false, // Drafts are not active by default
            allow_retake: false,
            max_retake_attempts: '',
            expiry_date: '',
            prerequisite_module_id: '',
        ...
        const checkForUnsavedChanges = () => {
            const programDataChanged = JSON.stringify(programData) !== JSON.stringify(initialProgramData);
            const materialsChanged = JSON.stringify(materials) !== JSON.stringify(initialMaterials);
            const preTestChanged = JSON.stringify(preTestQuestions) !== JSON.stringify(initialPreTestQuestions);
            const postTestChanged = JSON.stringify(postTestQuestions) !== JSON.stringify(initialPostTestQuestions);
            
            return programDataChanged || materialsChanged || preTestChanged || postTestChanged;
        };
        ...
        }, [programData, materials, preTestQuestions, postTestQuestions]);
        ...
        useEffect(() => {
            const handleNavigate = (event) => {
                if (hasUnsavedChanges && !event.detail.visit.url.includes('/admin/training-programs/create')) {
            ...
            const handleLogoutSubmit = (event) => {
                ...
                if (form.action && form.action.includes('/logout') && hasUnsavedChanges) {
                    event.preventDefault();
            ...
            document.addEventListener('inertia:before', handleNavigate);
            document.addEventListener('submit', handleLogoutSubmit);
            return () => {
                document.removeEventListener('inertia:before', handleNavigate);
                document.removeEventListener('submit', handleLogoutSubmit);
            };
        }, [hasUnsavedChanges]);
        ...
        const confirmNavigation = () => {
            ...
            if (pendingNavigation) {
                if (typeof pendingNavigation === 'function') {
                    ...
                    router.visit(pendingNavigation.url, pendingNavigation.options || {});
                }
        ...
        const validateStep = () => {
            ...
            if (currentStep === 1) {
                ...
                // Skip other validations in draft mode
                if (isDraftMode) {
                    return true;
                }
        ...
        const handleSubmit = async (isDraft = false) => {
            ...
            try {
                ...
                // Add program data - ensure title is non-empty
                formData.append('title', programData.title.trim());
                formData.append('description', programData.description);
                formData.append('duration_minutes', programData.duration_minutes || 60); // Default for drafts
                formData.append('passing_grade', programData.passing_grade);
                formData.append('category', programData.category);
                formData.append('difficulty', programData.difficulty);
                formData.append('is_draft', isDraft ? 1 : 0); // Add draft flag
                formData.append('is_active', isDraft ? 0 : (programData.is_active ? 1 : 0)); // Drafts are never active
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                <div className="flex gap-8">
                    ...
                    <div className="w-2/3 space-y-6">
                        ...
                        <div className="group">
                            <label className="text-xs font-bold uppercase text-slate-500 tracking-wider mb-2">Description</label>
                            <textarea 
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <div className="group">
                                ...
                                <div className="relative">
                                    <BarChart3 className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors" size={18} />
                                    <select
        ...
        return (
            <AdminLayout user={auth.user}>
                    ...
                    <div className="bg-slate-50 px-12 py-6 border-t border-slate-100 flex justify-between items-center">
                        ...
                        <div className="flex gap-4">
                            ...
                            <button 
                                ...
                                disabled={loading}
                                className="flex items-center gap-2 px-6 py-3 bg-slate-200 text-slate-700 rounded-xl font-bold text-sm hover:bg-slate-300 transition disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {loading && isDraftMode ? (
                                    <>
                                        <span className="animate-spin">⏳</span> Saving Draft...
                                    </>
                                ) : (
                                    <> 💾 Save Draft </>
                                )}
                ...
                {/* Unsaved Changes Confirmation Pop-up (Tailwind) */}
                <Modal show={showUnsavedChangesModal} onClose={cancelNavigation} maxWidth="lg" panelClassName="max-h-[480px]">
                    <div className="p-6 text-center">
                        <div className="mx-auto w-20 h-20 bg-gradient-to-br from-amber-400 to-orange-500 rounded-2xl flex items-center justify-center shadow-lg mb-4">
                            <AlertCircle className="w-10 h-10 text-white" />
                        </div>

                        <h3 className="text-2xl font-bold text-slate-900 mb-2">Perubahan belum disimpan</h3>
                        <p className="text-slate-600 mb-6 px-4">Anda memiliki perubahan yang belum disimpan. Jika meninggalkan halaman, perubahan ini akan hilang.</p>

                        <div className="flex gap-3 justify-center">
                            <button
                                onClick={cancelNavigation}
                                className="px-6 py-3 bg-slate-100 text-slate-700 rounded-2xl font-semibold hover:bg-slate-200 transition"
                            >
                                Tetap di Halaman
                            </button>
                            <button
                                onClick={confirmNavigation}
                                className="px-6 py-3 bg-red-600 text-white rounded-2xl font-semibold hover:bg-red-700 transition"
                            >
                                Tinggalkan Halaman
                            </button>
                        </div>

                        <p className="text-xs text-slate-400 mt-4">Tip: Anda bisa menyimpan draft terlebih dahulu jika ingin kembali mengedit nanti.</p>
                    </div>
                </Modal>
            </AdminLayout>
        );
